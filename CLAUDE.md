...
│   ├── usage/page.tsx             # Server component: the logged-in user's own usage summary + event table
│   └── api/
...
└── components/
    ...
    └── UserBar.tsx                # Server component: shows "Hi <email local-part>" + Usage link + logout
```
...
`src/app/usage/page.tsx` shows a user their own usage: an all-time summary (`prisma.usageEvent.groupBy` by `filter` and by `sortKey`, independent of any display cap) plus a chronological table of their most recent 100 events. It's a server component (like `UserBar.tsx`) — calls `auth()` and queries Prisma directly, no separate GET endpoint. Already gated by `middleware.ts`'s default-deny; it also calls `redirect("/login")` itself as defense-in-depth, matching `/api/usage`'s explicit 401 check. Reachable via the "Usage" link in `UserBar.tsx` (visible on every page) and links back to `/` via "← Back to crawler".

## Testing

Tests use Vitest + React Testing Library. `src/lib/*.test.ts` covers the pure sort/filter/parser/auth logic (parser tests use the fixture at `src/lib/__fixtures__/hn-sample.html`; `verifyCredentials.test.ts` mocks `@/lib/prisma` and `bcryptjs`); `src/app/api/crawl/route.test.ts`, `src/app/api/register/route.test.ts`, and `src/app/api/usage/route.test.ts` mock `fetch`/`@/lib/prisma`/`@/auth` to test each route's success/error paths; `src/components/UserBar.test.tsx` and `src/app/usage/page.test.tsx` mock `@/auth` (and `@/lib/prisma`, `next/navigation`'s `redirect` for the usage page) and render the resolved server component directly (`render(await UserBar())` / `render(await UsagePage())`) — the redirect mock throws, matching real Next.js behavior, so `await expect(UsagePage()).rejects.toThrow()` verifies the no-session path without any unreachable-code workarounds in the component itself; `src/app/page.test.tsx` renders the real page + table, uses a URL-aware `fetch` mock (distinguishing `/api/crawl` from `/api/usage`, since sort/filter interactions now also hit `/api/usage`), and drives crawl/sort/filter interactions with `@testing-library/user-event`, asserting on the logged usage payloads. Full Auth.js `signIn`/middleware/cookie flows are intentionally not covered by automated tests — verify those manually (register → login → confirm `/`, `/api/crawl`, and `/usage` are gated → logout). CI (`.github/workflows/ci.yml`) runs `prisma generate`, lint, test, and build on every push/PR to `main`, using dummy `DATABASE_URL`/`AUTH_SECRET` values (no real DB needed since Prisma is mocked in tests and `prisma generate` doesn't require connectivity).
